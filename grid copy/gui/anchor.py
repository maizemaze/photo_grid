...
class PnAnchor(QWidget):
    ...
    def mousePressEvent(self, event):
        ...
        if self.rec_acr_c.contains(pos):
            ...
            if event.button() == Qt.RightButton:
                self.grid.delAnchorRatio(dim=1, index=self.idx_click)
        elif self.rec_acr_r.contains(pos):
            self.idx_click = (np.abs(self.y_acr_r-pos.y())).argmin()
            if event.button() == Qt.RightButton:
                self.grid.delAnchorRatio(dim=0, index=self.idx_click)
    ...
    def mouseMoveEvent(self, event):
        ...
        if event.buttons() == Qt.LeftButton:
            if self.rec_acr_c.contains(pos):
                self.grid.modAnchorRatio(dim=1, index=self.idx_click, value=(
                    pos.x()-correctX)/self.size_img.width())
            elif self.rec_acr_r.contains(pos):
                self.grid.modAnchorRatio(dim=0, index=self.idx_click, value=(
    ...
    def paintEvent(self, paint_event):
        ...
        '''sort array'''
        self.acr_c, self.acr_r = self.grid.getAnchorRatio()
        '''size info'''
